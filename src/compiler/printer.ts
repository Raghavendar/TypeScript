...
namespace ts {
    export interface PrintContext {
        ...
        /**
         * Emits a node with leading and trailing comments.
         */
        emitNodeWithComments?(hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void): void;

        /**
         * Emits the body of a node with detached comments.
         */
        emitBodyWithDetachedComments?(node: Node, detachedRange: TextRange, emitCallback: (node: Node) => void): void;

        /**
         * Emits the trailing comments of a position.
         */
    ...
    let sourceMap: SourceMapWriter;
    let comments: CommentWriter;
    let nameGenerator: NameGenerator;
    let bundledHelpers: Map<boolean>;
    let emitNodeWithSubstitution: (hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void) => void;
    let emitNodeWithNotification: (hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void) => void;
    let emitNodeWithComments: (hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void) => void;
    let emitBodyWithDetachedComments: (node: Node, detachedRange: TextRange, emitCallback: (node: Node) => void) => void;
    let emitTrailingCommentsOfPosition: (pos: number) => void;

    export function printNode(_resolver: EmitResolver, _host: EmitHost, context: PrintContext, hint: EmitHint, node: Node, sourceFile: SourceFile, isBundledEmit: boolean) {
        const savedResolver = resolver;
        const savedHost = host;
        const savedCompilerOptions = compilerOptions;
        const savedLanguageVersion = languageVersion;
        const savedWriter = writer;
        const savedSourceMap = sourceMap;
        const savedComments = comments;
        const savedNameGenerator = nameGenerator;
        const savedCurrentSourceFile = currentSourceFile;
        const savedIsOwnFileEmit = isOwnFileEmit;
        const savedBundledHelpers = bundledHelpers;
        const savedEmitNodeWithSubstitution = emitNodeWithSubstitution;
        const savedEmitNodeWithNotification = emitNodeWithNotification;

        resolver = _resolver;
        host = _host;
        currentSourceFile = sourceFile;
        isOwnFileEmit = !isBundledEmit;
        emitNodeWithSubstitution = context && context.emitNodeWithSubstitution;
        emitNodeWithNotification = context && context.emitNodeWithNotification;

        const newLine = host.getNewLine();
        compilerOptions = host.getCompilerOptions();
        languageVersion = getEmitScriptTarget(compilerOptions);
        writer = context && context.writer || createTextWriter(newLine);
        sourceMap = context && context.sourceMap || createSourceMapWriter(host, writer);
        comments = context && context.comments || createCommentWriter(writer, compilerOptions, newLine, sourceMap.emitPos);
        nameGenerator = createNameGenerator(resolver, getTextOfNode);
        ...
        isOwnFileEmit = savedIsOwnFileEmit;
        emitNodeWithSubstitution = savedEmitNodeWithSubstitution;
        emitNodeWithNotification = savedEmitNodeWithNotification;
        compilerOptions = savedCompilerOptions;
        languageVersion = savedLanguageVersion;
        writer = savedWriter;
        sourceMap = savedSourceMap;
        comments = savedComments;
        nameGenerator = savedNameGenerator;
        bundledHelpers = savedBundledHelpers;
    }
    ...
    function pipelineEmitWithNotification(hint: EmitHint, node: Node) {
        return emitNodeWithNotification
            ? emitNodeWithNotification(hint, node, pipelineEmitWithComments)
    ...
    function pipelineEmitWithComments(hint: EmitHint, node: Node) {
        ...
        return hint !== EmitHint.SourceFile && emitNodeWithComments
    ...
    function pipelineEmitWithSourceMap(hint: EmitHint, node: Node) {
        ...
        return hint !== EmitHint.SourceFile && hint !== EmitHint.IdentifierName
            ? emitNodeWithSourceMap(hint, node, pipelineEmitWithSubstitution)
     ...
     * pipelineEmitUnspecified (when picking a more specific context).
     */
    function pipelineEmitWithSubstitution(hint: EmitHint, node: Node) {
        return emitNodeWithSubstitution
            ? emitNodeWithSubstitution(hint, node, pipelineEmitWithHint)
    ...
    /**
     * Emits a node with the SourceFile hint.
     *
     * NOTE: Do not call this method directly. It is part of the emit pipeline
     * and should only be called indirectly from pipelineEmitWithHint.
     */
    ...
    /**
     * Emits a node with the IdentifierName hint.
     *
     * NOTE: Do not call this method directly. It is part of the emit pipeline
     * and should only be called indirectly from pipelineEmitWithHint.
     */
    ...
    /**
     * Emits a node with the Unspecified hint.
     *
     * NOTE: Do not call this method directly. It is part of the emit pipeline
     * and should only be called indirectly from pipelineEmitWithHint.
     */
    ...
    /**
     * Emits a node with the Expression hint.
     *
     * NOTE: Do not call this method directly. It is part of the emit pipeline
     * and should only be called indirectly from pipelineEmitWithHint.
     */
    ...
    function emitNodeWithSourceMap(hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void): void {
        sourceMap.emitNodeWithSourceMap(hint, node, emitCallback);
    }

    function emitTokenWithSourceMap(node: Node, token: SyntaxKind, tokenStartPos: number, emitCallback: (token: SyntaxKind, tokenStartPos: number) => number): number {
        return sourceMap.emitTokenWithSourceMap(node, token, tokenStartPos, emitCallback);
    }

    // function emitNodeWithComments(hint: EmitHint, node: Node, emitCallback: (hint: EmitHint, node: Node) => void): void {
    //     comments.emitNodeWithComments(hint, node, emitCallback);
    // }

    // function emitBodyWithDetachedComments(node: Node, detachedRange: TextRange, emitCallback: (node: Node) => void): void {
    //     comments.emitBodyWithDetachedComments(node, detachedRange, emitCallback);
    // }

    // function emitTrailingCommentsOfPosition(pos: number): void {
    //     comments.emitTrailingCommentsOfPosition(pos);
    // }
    ...
    function pushNameGenerationScope(): void {
        nameGenerator.pushNameGenerationScope();
    }

    function popNameGenerationScope(): void {
        nameGenerator.popNameGenerationScope();
    }
